...
if __name__=='__main__':
    ...
    ip_cnt, op_cnt, hid_cnt=784, 10, 256

    """设置网络权重"""
    w1 = nn.Parameter(tf.randn((ip_cnt,hid_cnt),requires_grad=True)*0.01)
    b1 = nn.Parameter(tf.zeros(hid_cnt,requires_grad=True))

    w2 = nn.Parameter(tf.randn((hid_cnt,op_cnt),requires_grad=True)*0.01)
    b2 = nn.Parameter(tf.zeros(op_cnt,requires_grad=True))

    params = [w1, b1, w2, b2]
    loss = nn.CrossEntropyLoss()
    updater = tf.optim.SGD(params, lr=lr)
